...
from app.services.user_service import get_memory_user, set_memory_refresh_hash, upsert_google_user
...
async def google_login(
    ...
    if db is None:
        set_memory_refresh_hash(user["id"], hash_token(refresh_token))
    else:
...
async def refresh_token(
    ...
    user = get_memory_user(user_id) if db is None else await db.users.find_one({"_id": ObjectId(user_id)})
    ...
    if db is None:
        set_memory_refresh_hash(user_id, hash_token(new_refresh_token))
    else:
        await db.users.update_one(
