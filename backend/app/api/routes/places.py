...
async def list_places_route(
    ...
    places = await list_places(api_base_url=api_base_url, db=db, search=search, category=category, sort=sort, language=language)
...
async def get_place_route(
    ...
    api_base_url = str(request.base_url).rstrip("/")
    language = normalize_language(request.headers.get("accept-language"))
    return api_ok(await get_place(api_base_url=api_base_url, place_id=place_id, db=db, language=language))
